  ...
  while (true) {
    ...
    if (channelId === "telegram") {
      ...
      const result = await setupSlack({
        existing: isConfigured ? (existing as unknown as SlackChannelConfig) : undefined,
      });
      (next.channels as Record<string, unknown>).slack = result.slackConfig;
      // Merge speech config if provided
      if (result.speechConfig.stt) {
        next.speech = next.speech ?? { stt: { provider: null, providers: {} }, tts: { provider: null, providers: {} } };
        next.speech.stt = { provider: result.speechConfig.stt.provider, providers: { [result.speechConfig.stt.provider]: { apiKey: result.speechConfig.stt.apiKey } } };
      }
      if (result.speechConfig.tts) {
        next.speech = next.speech ?? { stt: { provider: null, providers: {} }, tts: { provider: null, providers: {} } };
        next.speech.tts = { provider: result.speechConfig.tts.provider, providers: { [result.speechConfig.tts.provider]: { voice: result.speechConfig.tts.voice } } };
      }
      changed = true;
    }
  }
